...
const initializeProfileTotals = (profilesArray, donationsArray) => {
	donationsArray.forEach(donation => {
			...
			if (profileIndex !== -1) {
					updateProfileTotalDonations(profilesArray, profileIndex, donation.amount);
			}
	});
};
...
const updateProfileTotalDonations = (profilesArray, profileIndex, donationAmount) => {
	profilesArray[profileIndex].total += donationAmount;
...
const addDonations = (donationArguments, profilesArray, donationsArray) => {
	...
	const updatedProfile = updateProfileTotalDonations(profilesArray, profileIndex, donation.amount);
